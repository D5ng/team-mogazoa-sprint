...
export default function ProductContents({
  ...
  return (
    <div className="relative w-[calc(100%-355px-40px)] text-white tablet:w-[51.478vw] mobile:w-full mobile:mt-5">
      ...
      <div className="flex justify-between mt-2.5 gap-5">
        <div className="flex gap-[10px] mobile:w-full mobile:justify-between">
          <h2 className="text-2xl font-semibold tablet:text-xl break-all">
            {name}
          </h2>
          <LikeButton isFavorite={isFavorite} productId={id} />
        </div>
        <div className="flex gap-x-2.5 mobile:absolute mobile:top-0 mobile:right-0">
          {isCreatedProductUser && <ProductDeleteButton productId={id} />}
      ...
      <div className="flex gap-x-5 tablet:gap-[15px] mobile:flex-col mobile:gap-x-0 mobile:gap-[15px]">
        <ReviewButton
          productId={id}
